...
%%
p_merge2=zeros(2256, 1);
options=optimoptions('fmincon', 'Display', 'notify');
merger=2;
for k=1:94
    k
    j=1+24*(k-1);
p_merge2(j:(j+23), 1)=fmincon(@obj3, p(j:(j+23), 1),[],[],[] ,[],zeros(24,1), ones(24, 1),[], options);
end

change_p2=(p_merge2-p)./p;
change_pMat2=reshape(change_p2, [24, 94]);
share_merge2=share(p_merge2);
change_s2=(share_merge2-s_jt)./s_jt;
change_sMat2=reshape(change_s2, [24, 94]);

table2=latex([(1:24)', mean(change_pMat1*100,2), mean(change_sMat1*100,2),mean(change_pMat2*100,2), mean(change_sMat2*100,2)],'%i','%.2f','%.2f','%.2f','%.2f', 'nomath');
