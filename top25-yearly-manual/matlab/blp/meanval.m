...
%%% Description:
%%% - 1) for given value of theta
%%% - 2) compute nonlinear part of utility from parameters
%%% - 3) contraction mapping for mean utility
...
% 2)
expmu = exp(mufunc(x2,theta2w)); %%% FUNCTION mufunc
norm = 1;
avgnorm = 1;
...
while norm > tol*10^(flag*floor(i/50)) & avgnorm > 1e-3*tol*10^(flag*floor(i/50))
   ...
   mval = mvalold.*s_jt./mktsh(mvalold,expmu); %%% FUNCTION mktsh
